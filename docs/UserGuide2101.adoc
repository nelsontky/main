...
[NOTE]
`PREFIX` `u/` lists all used coupons (including <<Archiving a coupon: `archive`, archived>> coupons that have been used
...
* To copy the second coupon in the current list of coupons to send it to someone.

Step 1. Key in `copy 2` in the <<UserInterface, Command Input Box>> to copy the second coupon to your clipboard.

image::UG_Copy_1.png[]


Step 2. You will see a message in the <<UserInterface, Command Result Box>> indicating that the coupon is copied to your clipboard.

image::UG_Copy_2.png[width="80%"]


Step 3. Simply kbd:[Ctrl] + kbd:[V] to paste it (kbd:[Cmd] + kbd:[V] for macOS) and send it to someone!

image::UG_Copy_3.png[]


* To copy the third coupon in the resulting list of coupons after <<Finding coupons by name: `find`, `find`>> command to
send it to someone.

Step 1. Key in `find food` in the <<UserInterface, Command Input Box>> to find all coupons with the keyword "food" in
their names.

image::UG_Copy_4.png[width="70%"]


Step 2. You will see a message in the <<UserInterface, Command Result Box>> indicating the number of coupons that has
names with the keyword "food".

image::UG_Copy_5.png[width=60%]


Step 3. Key in `copy 3` in the <<UserInterface, Command Input Box>> to copy the second coupon to your clipboard.

image::UG_Copy_6.png[]


Step 4. You will see a message in the <<UserInterface, Command Result Box>> indicating that the coupon is copied to your
clipboard.

image::UG_Copy_7.png[width="80%"]


Step 5. Simply kbd:[Ctrl] + kbd:[V] to paste it (kbd:[Cmd] + kbd:[V] for macOS) and send it to someone!
...
[WARNING]
Do not try entering this command with `MAX_INT` value as <<index, INDEX>>.
...
* `list` +
`share 2` +
Shares the second coupon in the resulting coupon list of the
<<_listing_coupons_list, `list`>> command and save
it as a `.png` file.
* `find chicken` +
`share 1` +
