...
<<Unarchiving a coupon: `unarchive` (Chester), unarchived>> on purpose))
...
* `list a/` +
Shows a list of all archived coupons in Coupon Stash. The <<CommandResultBox, Command Result Box>> will show a message
indicating that all archived coupons are listed.

* `list u/` +
Shows a list of all used coupons in Coupon Stash. The <<CommandResultBox, Command Result Box>> will show a message
indicating that all used coupons are listed.
// end::list[]
...
. Key in `copy 3` in the <<CommandInputBox, Command Input Box>> to copy the second coupon to your clipboard.
+
