...
The `list` command shows you a list of a type of coupon in your Coupon Stash. +
...
* The `PREFIX` `a/` lists all of your archived coupons and `u/` lists all of your used coupons. If left empty, it lists
all of your active coupons (coupons that are not fully used/ not archived (except for coupon(s) that are
<<Unarchiving a coupon: `unarchive`, unarchived>> on purpose))

****

[NOTE]
`PREFIX` `u/` lists all used coupons (including <<Archiving a coupon: `archive` (Chester), archived>> coupons that have been used
...
. Use the `saved` command to view your savings!
...
The `expiring` command shows a list of all your coupons expiring on a specified date or during a
...
The `copy` command copies a coupon as an <<Adding a coupon: `add`, `add`>> command to your
...
[[prevExample]]
* `copy 2` +
Copies the second coupon in the current list to an <<Adding a coupon: `add`, `add`>> command and copies it to
your clipboard.

** Here are instructions for copying the second coupon in the current list of coupons to send it to someone:

. Key in `copy 2` in the <<UserInterface, Command Input Box>> to copy the second coupon to your clipboard.
+
.Example of keying the `copy 2` command into the Command Input Box
image::UG_copy_1.png[width=60%]

. You will see a message in the <<UserInterface, Command Result Box>> indicating that the coupon is copied to your
clipboard.
+
.Example of result message of the <<Copying a coupon: `copy` (Denise) , `copy`>> command in the Command Result Box
image::UG_copy_2.png[width="40%"]

. Simply kbd:[Ctrl] + kbd:[V] to paste it (kbd:[Cmd] + kbd:[V] for macOS) and send it to someone!
+
.Example of sharing the copied coupon through a messaging application
image::UG_copy_3.png[width=50%]


* `find food` +
`copy 3` +
Copies the third coupon in the resulting coupon list of the
<<Finding coupons by name: `find`, `find`>> command to an <<Adding a coupon: `add`, `add`>> command
and copies it to your clipboard.

** Here are the instructions for copying the third coupon in the resulting list of coupons after <<Finding coupons by name: `find`, `find`>> command to
send it to someone:

. Key in `find food` in the <<UserInterface, Command Input Box>> to find all coupons with the keyword "food" in
their names.
+
.Example of keying the `find food` command into the Command Input Box
image::UG_copy_4.png[width="50%"]

. You will see a message in the <<UserInterface, Command Result Box>> indicating the number of coupons that has
...
. Key in `copy 3` in the <<UserInterface, Command Input Box>> to copy the second coupon to your clipboard. +
+
.Example of keying the `copy 3` command into the Command Input Box
image::UG_copy_6.png[width=70%]

. Repeat the last 2 steps from the <<prevExample, previous example>>!
// end::copy[]
...
Here are instructions for viewing your savings with the `share` command:
...
The `goto` command changes the month year displayed on the <<calendar, calendar>> to the specified month and year. +
...
Here are instructions for restoring previously entered command texts with the
kbd:[↑] and kbd:[↓] keys:
...
image::gifs/copy command.png[link=https://raw.githubusercontent.com/AY1920S2-CS2103T-F09-1/main/master/docs/images/gifs/copy%20command.gif]
